...
def clean_media_cache():
    ...
    total_size_before = sum(f.stat().st_size for f in media_dir.rglob("*") if f.is_file()) / (
        1024 * 1024
    )
    ...
    total_size_after = (
        sum(f.stat().st_size for f in media_dir.rglob("*") if f.is_file()) / (1024 * 1024)
        if media_dir.exists()
        else 0
    )
    ...
    print(
        f"Size: {total_size_before:.1f}MB → {total_size_after:.1f}MB (-{total_size_before - total_size_after:.1f}MB)"
    )
